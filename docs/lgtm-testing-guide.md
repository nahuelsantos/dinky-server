...
```bash
# Test Prometheus API
curl -s "http://192.168.1.129:9090/api/v1/query?query=up" | jq '.data.result | length'

# Check all targets status
curl -s "http://192.168.1.129:9090/api/v1/targets" | jq '.data.activeTargets[] | {job: .labels.job, instance: .labels.instance, health: .health}'

# Check unhealthy targets
curl -s "http://192.168.1.129:9090/api/v1/targets" | jq '.data.activeTargets[] | select(.health != "up") | {job: .labels.job, health: .health, lastError: .lastError}'

# Test website monitoring (blackbox-exporter)
curl -s "http://192.168.1.129:9090/api/v1/query?query=probe_success" | jq '.data.result[] | {instance: .metric.instance, job: .metric.job, status: .value[1]}'

# Check website response times
curl -s "http://192.168.1.129:9090/api/v1/query?query=probe_duration_seconds" | jq '.data.result[] | {instance: .metric.instance, duration: .value[1]}'

# Check HTTP status codes
curl -s "http://192.168.1.129:9090/api/v1/query?query=probe_http_status_code" | jq '.data.result[] | {instance: .metric.instance, code: .value[1]}'

# Check container metrics
curl -s "http://192.168.1.129:9090/api/v1/query?query=container_memory_usage_bytes" | jq '.data.result | length'

# Check node metrics (hardware)
curl -s "http://192.168.1.129:9090/api/v1/query?query=node_load1" | jq '.data.result[] | {instance: .metric.instance, load: .value[1]}'

# Test specific service metrics (example: contact-api)
curl -s "http://192.168.1.129:9090/api/v1/query?query=go_memstats_alloc_bytes{job=\"contact-api\"}" | jq '.data.result[] | {job: .metric.job, memory: .value[1]}'
```
...
curl -s "http://192.168.1.129:3100/ready"

# Check Loki metrics
curl -s "http://192.168.1.129:3100/metrics" | grep -c "loki_"

# Query recent logs (last hour) - Cross-platform compatible
NOW=$(date +%s) && START=$((NOW - 3600))
curl -s -G "http://192.168.1.129:3100/loki/api/v1/query_range" \
...
curl -s "http://192.168.1.129:3100/loki/api/v1/labels" | jq '.data[]'

# Check label values for specific label
curl -s "http://192.168.1.129:3100/loki/api/v1/label/container/values" | jq '.data[]'
...
```bash
# Test Tempo API
curl -s "http://192.168.1.129:3200/ready"

# Check Tempo metrics
curl -s "http://192.168.1.129:3200/metrics" | grep -c "tempo_"

# Check Tempo configuration
curl -s "http://192.168.1.129:3200/status/config" | jq '.distributor'

# Note: Port 4316 is OTLP protocol (not HTTP), so curl will show protocol error - this is expected
# curl "http://192.168.1.129:4316"  # Expected: HTTP/0.9 response (normal for OTLP)
```
...
curl -s "http://192.168.1.129:13133/"

# Check OTEL Collector metrics (Prometheus format)
curl -s "http://192.168.1.129:8889/metrics" | grep -c "otelcol"

# Check specific OTEL metrics
curl -s "http://192.168.1.129:8889/metrics" | grep "otelcol_receiver_accepted_spans"

# Check zpages (human-readable status)
curl -s "http://192.168.1.129:55679/debug/tracez"

# Check OTEL pipeline status
curl -s "http://192.168.1.129:55679/debug/pipelinez"
```
...
```bash
# Test Grafana API
curl -s "http://192.168.1.129:3000/api/health" | jq '.'

# List dashboards (requires auth)
curl -s -u "admin:Etching-Backspace1-Poach" "http://192.168.1.129:3000/api/search" | jq '.[] | {title: .title, uid: .uid}'

# Check datasources configuration
curl -s -u "admin:Etching-Backspace1-Poach" "http://192.168.1.129:3000/api/datasources" | jq '.[] | {name: .name, type: .type, uid: .uid, url: .url}'

# Test Prometheus datasource connectivity
curl -s -u "admin:Etching-Backspace1-Poach" "http://192.168.1.129:3000/api/datasources/proxy/1/api/v1/query?query=up" | jq '.data.result | length'

# Check dashboard by UID
curl -s -u "admin:Etching-Backspace1-Poach" "http://192.168.1.129:3000/api/dashboards/uid/contact-api" | jq '.dashboard.title'
```
...
curl -s "http://192.168.1.129:9093/api/v1/status" | jq '.'

# Check active alerts
curl -s "http://192.168.1.129:9093/api/v1/alerts" | jq '.data[] | {alertname: .labels.alertname, state: .status.state, summary: .annotations.summary}'

# Check alert rules
curl -s "http://192.168.1.129:9093/api/v1/rules" | jq '.data.groups[] | {name: .name, rules: (.rules | length)}'

# Test silence creation
curl -s "http://192.168.1.129:9093/api/v1/silences" | jq '.'

# Check Alertmanager configuration
curl -s "http://192.168.1.129:9093/api/v1/status" | jq '.data.configYAML'
```
...
curl -s "http://192.168.1.129:3002/metrics" | head -10

# Check if Prometheus is scraping contact-api
curl -s "http://192.168.1.129:9090/api/v1/query?query=up{job=\"contact-api\"}" | jq '.data.result[] | {instance: .metric.instance, status: .value[1]}'

# Verify Go runtime metrics
curl -s "http://192.168.1.129:9090/api/v1/query?query=go_goroutines{job=\"contact-api\"}" | jq '.data.result[] | {goroutines: .value[1]}'

# Check memory usage
curl -s "http://192.168.1.129:9090/api/v1/query?query=go_memstats_alloc_bytes{job=\"contact-api\"}" | jq '.data.result[] | {memory_bytes: .value[1]}'
```
...
curl -s "http://192.168.1.129:8889/metrics" | grep "otelcol_receiver_accepted_metric_points"

# Test OTEL Collector exporting to Tempo
curl -s "http://192.168.1.129:8889/metrics" | grep "otelcol_exporter_sent_spans"

# Check OTEL Collector pipeline health
curl -s "http://192.168.1.129:55679/debug/pipelinez" | grep -A5 "traces"
```
...
for service in "${SERVICES[@]}"; do
    ...
    if curl -s --connect-timeout 5 "http://192.168.1.129:$port" >/dev/null 2>&1; then
...
# Prometheus targets
TARGETS=$(curl -s "http://192.168.1.129:9090/api/v1/query?query=up" 2>/dev/null | jq -r '.data.result | length' 2>/dev/null)
echo "📊 Prometheus monitoring $TARGETS targets"

# Check unhealthy targets
UNHEALTHY=$(curl -s "http://192.168.1.129:9090/api/v1/targets" 2>/dev/null | jq -r '.data.activeTargets[] | select(.health != "up") | .labels.job' 2>/dev/null | wc -l)
...
# Loki logs (last hour)  
NOW=$(date +%s) && START=$((NOW - 3600))
LOGS=$(curl -s -G "http://192.168.1.129:3100/loki/api/v1/query_range" \
  --data-urlencode 'query={job="varlogs"}' \
  --data-urlencode "start=${START}000000000" \
...
# Check Loki labels
LOKI_LABELS=$(curl -s "http://192.168.1.129:3100/loki/api/v1/labels" 2>/dev/null | jq -r '.data | length' 2>/dev/null)
echo "🏷️  Loki has $LOKI_LABELS different labels"

# OTEL Collector metrics
OTEL_METRICS=$(curl -s "http://192.168.1.129:8889/metrics" 2>/dev/null | grep -c "otelcol" 2>/dev/null || echo "0")
echo "🔄 OTEL Collector has $OTEL_METRICS metrics"

# Website monitoring
WEBSITES_UP=$(curl -s "http://192.168.1.129:9090/api/v1/query?query=probe_success{job=\"website-probes\"}" 2>/dev/null | jq -r '.data.result[] | select(.value[1] == "1") | .metric.instance' 2>/dev/null | wc -l)
WEBSITES_TOTAL=$(curl -s "http://192.168.1.129:9090/api/v1/query?query=probe_success{job=\"website-probes\"}" 2>/dev/null | jq -r '.data.result | length' 2>/dev/null)
...
CONTACT_API_UP=$(curl -s "http://192.168.1.129:9090/api/v1/query?query=up{job=\"contact-api\"}" 2>/dev/null | jq -r '.data.result[0].value[1]' 2>/dev/null)
if [ "$CONTACT_API_UP" = "1" ]; then
    echo "✅ Contact API is integrated and monitored"
    CONTACT_MEMORY=$(curl -s "http://192.168.1.129:9090/api/v1/query?query=go_memstats_alloc_bytes{job=\"contact-api\"}" 2>/dev/null | jq -r '.data.result[0].value[1]' 2>/dev/null)
...
DASHBOARDS=$(curl -s -u "admin:Etching-Backspace1-Poach" "http://192.168.1.129:3000/api/search" 2>/dev/null | jq -r '. | length' 2>/dev/null)
echo "📊 Grafana has $DASHBOARDS dashboards configured"

echo
echo "4. Dashboard URLs:"
echo "🌐 Grafana: http://192.168.1.129:3000 (admin/Etching-Backspace1-Poach)"
echo "🔥 Prometheus: http://192.168.1.129:9090"
echo "📊 Alertmanager: http://192.168.1.129:9093"
echo "🐳 Portainer: http://192.168.1.129:9000"
echo "📈 Contact API: http://192.168.1.129:3002"

echo
echo "5. Key Grafana Dashboards:"
echo "   • Contact API Dashboard: http://192.168.1.129:3000/d/contact-api"
echo "   • Website Monitoring: http://192.168.1.129:3000/d/website-monitoring"
echo "   • Server Hardware Health: http://192.168.1.129:3000/d/server-hardware-health"
...
- Check datasource UID configuration:
  ```bash
  curl -s -u "admin:Etching-Backspace1-Poach" "http://192.168.1.129:3000/api/datasources" | jq '.[] | {name: .name, uid: .uid}'
  ```
- Verify Prometheus targets are UP:
  ```bash
  curl -s "http://192.168.1.129:9090/api/v1/targets" | jq '.data.activeTargets[] | select(.health != "up")'
  ```
...
- Test blackbox-exporter directly:
  ```bash
  curl -s "http://192.168.1.129:9115/probe?target=https://google.com&module=http_2xx"
  ```
...
- Verify alert rules are loaded:
  ```bash
  curl -s "http://192.168.1.129:9093/api/v1/rules"
  ```
- Check for silenced alerts:
  ```bash
  curl -s "http://192.168.1.129:9093/api/v1/silences"
  ```
...
curl -s "http://192.168.1.129:9090/api/v1/status/tsdb" | jq '.data.headStats'
```

#### Data Retention Checks
```bash
# Check Prometheus retention
curl -s "http://192.168.1.129:9090/api/v1/status/flags" | jq '.data["storage.tsdb.retention.time"]'
...
curl -s "http://192.168.1.129:3200/status/config" | jq '.ingester.lifecycler.ring.kvstore'
```
...
- **OTEL Collector debug:** `http://192.168.1.129:55679/debug/`
...
for port in 9090 3100 3200 3000 9093; do echo -n "Port $port: "; curl -s --connect-timeout 2 "http://192.168.1.129:$port" >/dev/null && echo "✅ UP" || echo "❌ DOWN"; done

# Check all Prometheus targets health
curl -s "http://192.168.1.129:9090/api/v1/targets" | jq -r '.data.activeTargets[] | "\(.labels.job): \(.health)"' | sort

# Quick Loki log count
NOW=$(date +%s) && START=$((NOW - 3600)) && curl -s -G "http://192.168.1.129:3100/loki/api/v1/query_range" --data-urlencode 'query={container!=""}' --data-urlencode "start=${START}000000000" --data-urlencode "end=${NOW}000000000" | jq '.data.result | length'
``` 